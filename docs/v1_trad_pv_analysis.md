...
### 유형 B: 일부 TP2=1 상품 — 초기연도 ACQSEXP 차감

검증된 상품: LA0216W, LA0217W (특정 담보)

```
# 초기 보험연도 (SETL=0이 속한 연도):
remaining = year_end_CM - current_CM
deduction = ACQSEXP1_BIZEXP × remaining / (insurance_year × 12)
SOFF_BF_TMRFND = APLY_PREM_ACUMAMT_BNFT - deduction

# 이후 보험연도:
SOFF_BF_TMRFND = APLY_PREM_ACUMAMT_BNFT
```
...
- TP2=1 상품 중 일부는 차감 적용, 일부는 미적용 (예: LA0214W는 미적용)
- 동일 상품(LA0216W)에서도 담보/계약에 따라 차감 여부가 다름
- 차감 적용 상품: LA0217W(대부분), LA0216W(일부), LA0211Z, LA0215R
- **8,427건** (전체 42,000 중)에서 SOFF ≠ APLY_PREM_ACUMAMT_BNFT
- 차감 조건의 정확한 결정 기준은 미파악 (IP_P_COV 필드만으로는 구분 불가)
...
| SOFF_BF_TMRFND | 167 | 222 | 42.9% (상품별 차감 미구현) |
