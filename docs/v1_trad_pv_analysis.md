...
| PREM_PAY_YN | II_INFRC | PAY_STCD=1 AND CTR_MM ≤ PTERM_MM ? 1 : 0 |
| ORIG_PREM | II_INFRC | GRNTPT_GPREM (상수) |
| DC_PREM | II_INFRC | round_half_up(GPREM × (1 - PREM_DC_RT1)) |
...
### PREM_PAY_YN — PAY_STCD 기반 (해결됨)

PAY_STCD (II_INFRC) = 1(납입중), 2(납입완료), 3(납입면제).
PAY_STCD ≠ 1이면 PREM_PAY_YN = 0. 42,000건 중 2건 예외(0.005%).

### DC_PREM — PREM_DC_RT1 할인 (해결됨)

`DC_PREM = round_half_up(GPREM × (1 - PREM_DC_RT1))`.
LA02155(2%), LA02158(2%), LA02079(0.5%) 등 552건에 적용.
42,000건 중 GPREM>0인 경우 25건만 반올림 경계 차이 (round-half-up vs banker's rounding).
...
| PREM_PAY_YN | 233 | 0 | **100%** (PAY_STCD 기반) |
| DC_PREM | 233 | 0 | **100%** (PREM_DC_RT1 적용) |
| PAD_PREM | 233 | 0 | **100%** (PAY_STCD 연쇄 해결) |
